...
about.html              About: intro, one "Soyuns" photo gallery (three staggered columns of 4 photos each plus one full-width wide photo; columns have equal height so the block is a clean rectangle), footer with local time
...
The About footer shows the time in Los Gatos, CA (`#local-time`, `America/Los_Angeles`). Known values still to replace: social links (`href="#"`), all case-study facts and copy.
