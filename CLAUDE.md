...
- **Fonts:** Newsreader (serif, headings, italic for section titles) and Inter (UI/body), plus Comfortaa (500) only for the `Soyun` brand name in the header, loaded from Google Fonts in each page's `<head>`. Keep that `<link>` identical across pages.
