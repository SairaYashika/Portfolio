...
- **Mouse pointer:** `html { cursor: url(images/cursor-sun.svg) 9 9, auto }` in `styles.css`. `cursor-sun.svg` is an 18px version of the owner's `logo-1b` sun (strokes thickened so they stay visible at that size, white halo so it shows on dark photos). Clickable elements (`a[href]`, `button`) use `images/cursor-sun-link.svg`: the same sun with its centre circle filled gold (`#F5B841`) instead of the pointing hand. Both fall back to the normal cursors. Cursor images must stay <= 128px.
