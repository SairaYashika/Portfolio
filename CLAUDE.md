...
- **Mouse pointer:** `html { cursor: url(images/cursor-sun.svg) 18 18, auto }` in `styles.css`. `cursor-sun.svg` is a 36px version of the owner's `logo-1b` sun (strokes thickened, white halo so it shows on dark photos). Links keep the browser's pointing hand on purpose. Cursor images must stay <= 128px.
